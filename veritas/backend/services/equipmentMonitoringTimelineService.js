...
export async function loadEquipmentMonitoringTimeline({
  equipmentId,
  clientId,
  days = 90
} = {}) {
  ...
  const alertHistory = await pool.query(`SELECT last_known_status, last_known_criteria, last_ticket_id, last_alert_at, updated_at
     FROM v_b_equipment_monitoring_alerts
     WHERE client_id = $1 AND equipment_id = $2::uuid
     ORDER BY updated_at DESC
     LIMIT 20`, [clientId, equipmentId]).catch(() => ({
    rows: []
  }));
  ...
  const monitoringTickets = (activity.tickets || []).filter(t => t.channel === "monitoring" || String(t.category || "") === "infrastructure");
  return {
