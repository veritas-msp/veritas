...
import { testHycuConnection } from "../../api/integrationConnectionTests";
import { showError } from "../../utils/toast";
import { useAppLocale } from "../../hooks/useAppGeneralSettings";
import { getHycuIntegrationModalCopy } from "./adminIntegrationModalsI18n";
...
function HycuTestResultModal({ result, error, onClose, copy }) {
          ...
          <div className={formStyles.content}>
            ...
            {isSuccess ? <div className={styles.kpiGrid}>
                <div className={styles.kpiCard}>
                  <div className={styles.kpiValue}>{result.jobsCount ?? 0}</div>
                  <div className={styles.kpiLabel}>{copy.jobs}</div>
                </div>
              </div> : null}
          </div>
        </div>
...
export default function HycuIntegrationModal({
  open,
  enabled,
  apiUrl,
  apiKey,
  ...
  const copy = useMemo(() => getHycuIntegrationModalCopy(locale), [locale]);
  ...
  const handleTest = async () => {
    if (!(apiUrl || "").trim() || (!(apiKey || "").trim() && (!(username || "").trim() || !(password || "").trim()))) {
    ...
    setTestError(null);
    try {
      const data = await testHycuConnection({
        apiUrl: (apiUrl || "").trim(),
        apiKey: apiKey || "",
  ...
  const renderConnection = () => <>
      <div className={styles.statusRow}>
        ...
        <label className={formStyles.switchWrap}>
          ...
          <span className={formStyles.switchTrack} aria-hidden><span className={formStyles.switchThumb} /></span>
        </label>
      </div>
      ...
      <div className={formStyles.fieldStack}>
        <div className={formStyles.field}>
          <label className={formStyles.label} htmlFor="hycu-api-url">{copy.apiUrl}</label>
          <input id="hycu-api-url" type="url" className={formStyles.input} value={apiUrl || ""} placeholder="https://hycu.example.com:8443" onChange={e => onApiUrlChange(e.target.value)} disabled={saving || testing} autoComplete="off" />
        </div>
        <div className={formStyles.field}>
          <label className={formStyles.label} htmlFor="hycu-api-key">{copy.apiKey}</label>
          <input id="hycu-api-key" type="password" className={formStyles.input} value={apiKey || ""} onChange={e => onApiKeyChange(e.target.value)} disabled={saving || testing} autoComplete="off" />
        </div>
        <div className={formStyles.field}>
          <label className={formStyles.label} htmlFor="hycu-username">{copy.username}</label>
          <input id="hycu-username" type="text" className={formStyles.input} value={username || ""} onChange={e => onUsernameChange(e.target.value)} disabled={saving || testing} autoComplete="off" />
        </div>
        <div className={formStyles.field}>
          <label className={formStyles.label} htmlFor="hycu-password">{copy.password}</label>
          <input id="hycu-password" type="password" className={formStyles.input} value={password || ""} onChange={e => onPasswordChange(e.target.value)} disabled={saving || testing} autoComplete="off" />
        </div>
        <div className={formStyles.field}>
          <label className={formStyles.label} htmlFor="hycu-verify-tls">{copy.verifyTls}</label>
          <select id="hycu-verify-tls" className={formStyles.input} value={String(verifyTls || "false")} onChange={e => onVerifyTlsChange(e.target.value)} disabled={saving || testing}>
  ...
  return createPortal(<>
      <div className={formStyles.overlay} onClick={saving || testing ? undefined : onClose} role="presentation">
        <div className={formStyles.shell} onClick={e => e.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="hycu-integration-modal-title">
          <div className={formStyles.accentBar} aria-hidden />
          <header className={formStyles.header}>
            <div className={formStyles.headerMain}>
              <div className={formStyles.headerIconWrap} aria-hidden>
              ...
              <div className={formStyles.headerText}>
                <p className={formStyles.eyebrow}>{copy.eyebrow}</p>
                <h2 className={formStyles.title} id="hycu-integration-modal-title">{copy.title}</h2>
          ...
          <footer className={formStyles.footer}>
            ...
            <div className={formStyles.footerActions}>
              ...
              <button type="button" className={formStyles.ghostBtn} onClick={onClose} disabled={saving || testing}>{copy.cancel}</button>
              <button type="button" className={formStyles.primaryBtn} onClick={onSave} disabled={saving || testing}>{saving ? copy.saving : copy.save}</button>
            </div>
          </footer>
        </div>
      </div>
      {showTestModal ? <HycuTestResultModal result={testResult} error={testError} onClose={() => setShowTestModal(false)} copy={copy} /> : null}
