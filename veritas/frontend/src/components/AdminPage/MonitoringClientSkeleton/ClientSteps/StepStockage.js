...
import RoleTagsSelect from "../../../EquipementPage/RoleTagsSelect";
import { STORAGE_ROLE_GROUPS, STORAGE_ROLE_OPTIONS, hasStorageRoles, normalizeStorageRoles } from "../../../EquipementPage/constants/storageRoleOptions";
const StepStockage = ({
  form,
  setForm,
  showTypeModal,
  ...
  const raidOptions = ["RAID 0", "RAID 1", "RAID 5", "RAID 6", "RAID 10", "RAID 50", "RAID 60", "SHR (Synology)", "SHR-2 (Synology)", "RAID F1", "Qtier (QNAP)", "RAID-TP (QNAP)", "RAID-Z (ZFS)", "RAID-Z2 (ZFS)", "RAID-Z3 (ZFS)", "Other"];
  const capaciteOptions = ["1 TB", "2 TB", "4 TB", "6 TB", "8 TB", "10 TB", "12 TB", "16 TB", "20 TB", "24 TB", "32 TB", "40 TB", "48 TB", "64 TB", "80 TB", "100 TB", "Other"];
      ...
      if (newNameIsUnique) {
        try {
          ...
          if (deleteResponse.ok) {
            ...
            console.error('Error deleting mapping CheckMK:', await deleteResponse.text());
          }
        } catch (error) {
          console.error('Error deleting mapping CheckMK:', error);
        }
      }
    }
  };
      ...
      try {
        ...
        if (response.ok) {
          ...
          (form.equipements?.NAS || []).forEach((item, idx) => {
            const matchingMapping = mappings.find(m => m.equipment_type === 'Stockage' && m.equipment_id === item.id);
            if (matchingMapping) {
  ...
  return <motion.div className={styles.stepContainer} initial={{
    ...
    ease: "circOut"
      ...
      <div className={styles.formSection}>
        ...
        <div className={styles.scrollable}>
          {sortedSites.length === 0 ? <div className={styles.emptyState}>
              <p className={styles.emptyStateTitle}>No sites or storage devices</p>
              <p className={styles.emptyStateDescription}>
                Add a site or device to get started.
              </p>
            ...
            marginBottom: '0.5rem'
              ...
              handleSiteDragOver(e, siteName);
                ...
                justifyContent: 'space-between'
                  ...
                  fontWeight: '500'
                        ...
                        {siteEquipments.length} device{siteEquipments.length > 1 ? 's' : ''}
                      </span>
                    </div>
                  </div>
                  <div style={{
              ...
              <div onClick={e => {
                ...
                <div style={{
                  ...
                  {equipment.type !== 'External hard drive' && <button onClick={e => {
                        e.stopPropagation();
                        setCheckmkMappingModal({
                          isOpen: true,
                          itemIndex: i
                        });
                      }} title={checkmkMappings[i] ? `Mapped to: ${checkmkMappings[i].checkmk_host_name}` : "Map with Check MK"} style={{
                        padding: '0.5rem',
                  ...
                  <button onClick={e => {
                        e.stopPropagation();
                        remove(i);
                      }} title="Delete this device" style={{
              ...
              {expandedItems.has(i) && <div style={{
                  ...
                  <div className={styles.formGrid} style={{
                    ...
                    <div className={styles.formField}>
                      <label htmlFor={`nas-nom-${i}`}>
                        {equipment.type === 'External hard drive' ? 'Device name' : 'NetBIOS'} <span style={{
                            color: 'red'
                          }}>*</span>
                      </label>
                  ...
                  {}
                  {equipment.type !== 'External hard drive' && <div className={styles.formGrid} style={{
                      gridTemplateColumns: '1fr 1fr 1fr'
                    }}>
                      <div className={styles.formField}>
                        <label htmlFor={`nas-fabricant-${i}`}>Brand</label>
                        <input id={`nas-fabricant-${i}`} value={equipment.fabricant} onChange={e => update(i, "fabricant", e.target.value)} />
                      </div>
                      <div className={styles.formField}>
                        <label htmlFor={`nas-modele-${i}`}>Model</label>
                        <input id={`nas-modele-${i}`} value={equipment.modele} onChange={e => update(i, "modele", e.target.value)} />
                      </div>
                      <div className={styles.formField}>
                        <label htmlFor={`nas-serie-${i}`}>Serial No.</label>
                  ...
                  {equipment.type !== 'External hard drive' && <div className={styles.formGrid} style={{
                      ...
                      <div className={styles.formField}>
                        <label htmlFor={`nas-garantie-${i}`}>Warranty (expiration)</label>
                        <input id={`nas-garantie-${i}`} type="date" value={equipment.expirationWarranty} onChange={e => update(i, "expirationWarranty", e.target.value)} />
                      </div>
                    </div>}
                  ...
                  {}
                  {equipment.type !== 'Backup robot' && equipment.type !== 'External hard drive' && <div className={styles.formGrid} style={{
                      gridTemplateColumns: '1fr 1fr 1fr'
                    }}>
                      <div className={styles.formField}>
                        <label htmlFor={`nas-disques-actuels-${i}`}>Disques actuels</label>
                        <input id={`nas-disques-actuels-${i}`} type="text" inputMode="numeric" pattern="[0-9]*" value={equipment.nbDisquesActuels} onChange={e => update(i, "nbDisquesActuels", e.target.value)} onKeyPress={e => {
                          if (!/[0-9]/.test(e.key)) {
                            e.preventDefault();
                          }
                        }} />
                      ...
                      <div className={styles.formField}>
                        ...
                        <input id={`nas-disques-max-${i}`} type="text" inputMode="numeric" pattern="[0-9]*" value={equipment.nbDisquesMax} onChange={e => update(i, "nbDisquesMax", e.target.value)} onKeyPress={e => {
                          if (!/[0-9]/.test(e.key)) {
                            e.preventDefault();
                          }
                        }} />
                      ...
                      <div className={styles.formField}>
                        <label htmlFor={`nas-capacite-${i}`}>Total capacity (GB)</label>
                        <input id={`nas-capacite-${i}`} type="text" inputMode="numeric" pattern="[0-9]*" value={equipment.capacite} onChange={e => update(i, "capacite", e.target.value)} onKeyPress={e => {
                          if (!/[0-9]/.test(e.key)) {
                            e.preventDefault();
                          }
                        }} />
                  ...
                  {(equipment.type === 'NAS' || equipment.type === 'SAN') && <div className={styles.formField} style={{
                      ...
                      <label>LUNs</label>
                      {(equipment.luns || []).length === 0 && <p className={styles.noLicences} style={{
                        margin: '0.5rem 0'
                      }}>No LUN configured</p>}
                        ...
                        marginBottom: 8
                          ...
                          gap: 4
                            ...
                            <select value={lun.role || "stockage"} onChange={e => updateLUN(i, lunIndex, "role", e.target.value)}>
                              ...
                              <option value="exploitation">Production</option>
                            </select>
                          </div>
                  ...
                  {equipment.type === 'Backup robot' && <div className={styles.formGrid}>
                      <div className={styles.formField} style={{
                        gridColumn: '1 / -1'
                      }}>
                        <label>RDX tape set</label>
                        <div>
                          {(equipment.cassettesRDX || []).length === 0 && <p className={styles.noLicences} style={{
                            margin: '0.5rem 0'
                          }}>No tape configured</p>}
                          <div style={{
                            display: 'grid',
                            ...
                            {(equipment.cassettesRDX || []).map((cassette, cassetteIndex) => <div key={cassetteIndex} className={styles.licenceItem} style={{
                              ...
                              borderRadius: 6
                                ...
                                <button type="button" onClick={() => removeCassetteRDX(i, cassetteIndex)} className={styles.deleteButton} title="Delete this tape" style={{
                                minWidth: 24,
                                alignSelf: 'center'
                              }}>
                                  <Icon icon="mdi:delete" width={20} height={20} />
      ...
      {}
      {showTypeModalState && createPortal(<div className={adminStyles.modalOverlay} onClick={() => setShowTypeModalState(false)}>
        ...
        maxWidth: '640px'
            ...
            gap: '0.75rem'
                ...
                <Icon icon="mdi:harddisk" className={adminStyles.modalIcon} style={{
              color: '#3b82f6'
            }} />
                <h3 style={{
              margin: 0,
      ...
      {}
      {checkmkMappingModal.isOpen && checkmkMappingModal.itemIndex !== null && NAS[checkmkMappingModal.itemIndex] && <CheckMKMappingModal isOpen={checkmkMappingModal.isOpen} onClose={() => setCheckmkMappingModal({
      isOpen: false,
      itemIndex: null
    })} equipmentName={NAS[checkmkMappingModal.itemIndex].nom} equipmentType="Stockage" equipmentIndex={checkmkMappingModal.itemIndex} equipmentId={NAS[checkmkMappingModal.itemIndex].id} clientId={form.id} requireService={false} onMappingSaved={mapping => {
      if (mapping) {
        setCheckmkMappings(prev => ({
          ...prev,
