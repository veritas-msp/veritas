...
import { ALL_PERMISSION_KEYS, defaultPermissionsForProfile, expandLegacyGrantedKeys, enforcePermissionDependencies } from "../config/permissionCatalog.js";
...
function normalizeGranted(set) {
  return enforcePermissionDependencies(set instanceof Set ? set : new Set(set));
}
async function writeProfileMatrix(client, name, grantedSet) {
  const normalized = normalizeGranted(grantedSet instanceof Set ? grantedSet : new Set(grantedSet));
  const values = [];
  const params = [];
  let i = 1;
...
export async function seedProfilePermissions(client, profileRow) {
  ...
  const currentAllowed = new Set(rows.filter(r => r.allowed).map(r => r.permission_key));
  const knownKeys = new Set(rows.map(r => r.permission_key));
  const missingKeys = ALL_PERMISSION_KEYS.filter(k => !knownKeys.has(k));
  if (preset) {
    if (!hasAnyViewPermission(currentAllowed) || setsEqual(currentAllowed, flagDefault)) {
      await writeProfileMatrix(client, name, preset);
      return;
    }
  }
  // Only fill brand-new catalog keys. Never re-force expansions onto keys the
  // admin already decided (e.g. sales.create must not re-enable home.create_sales).
  if (missingKeys.length > 0) {
    const legacyExpanded = expandLegacyGrantedKeys(currentAllowed);
    const merged = new Set(currentAllowed);
    const fillFrom = preset || flagDefault;
    for (const key of missingKeys) {
      if (legacyExpanded.has(key) || fillFrom.has(key)) merged.add(key);
    }
    await writeProfileMatrix(client, name, merged);
  }
}
