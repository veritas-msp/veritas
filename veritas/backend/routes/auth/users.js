...
const uploadAvatar = multer({
  ...
  fileFilter: (_req, file, cb) => {
    ...
    if (!allowed.includes(file.mimetype)) {
      return cb(new Error("Image format not allowed (JPG, PNG or WebP)."));
    }
    cb(null, true);
  }
});
...
router.post("/me/avatar/preset", verifyJWT, [body("presetId").isString().trim().notEmpty()], async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(400).json({
      error: errors.array().map(e => e.msg).filter(Boolean).join(" · ") || "Invalid data.",
      errors: errors.array()
    });
  }
...
router.get("/active", verifyJWT, async (req, res) => {
  try {
    const result = await pool.query(`SELECT u.id, u.username, u.email,
              hs.setting_value AS ticket_helpdesk_display_name,
              av.setting_value AS avatar_setting_raw
       FROM v_b_users u
       ${HELPDESK_DISPLAY_JOIN}
    ...
    res.status(500).json({
      error: "Error retrieving active users"
    });
  }
});
...
router.patch("/:id", verifyJWT, [param("id").isUUID(), body("role").optional(OPTIONAL_BODY).isIn(ALLOWED_USER_ROLES), body("profile").optional(OPTIONAL_BODY).isString().trim().isLength({
  ...
  min: 2,
  max: 50
}), body("email").optional(OPTIONAL_BODY).isEmail()], async (req, res) => {
  const errors = validationResult(req);
  ...
  try {
    if (is_active === true) {
      const target = await pool.query(`SELECT id, role, is_active FROM v_b_users WHERE id = $1`, [id]);
...
router.patch("/:id/password", verifyJWT, [param("id").isUUID(), body("newPassword").isString().isLength({
  min: 6
})], async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) return res.status(400).json({
...
router.delete("/:id", verifyJWT, requireRole("admin"), async (req, res) => {
  ...
  try {
    const target = await pool.query(`SELECT id, role FROM v_b_users WHERE id = $1`, [id]);
    ...
    if (target.rows[0].role === "admin") {
      const admins = await pool.query(`SELECT COUNT(*)::int AS total FROM v_b_users WHERE role = 'admin'`);
      if ((admins.rows[0]?.total || 0) <= 1) {
        return res.status(409).json({
          error: "Cannot delete the last administrator. At least one admin account must remain."
        });
      }
    }
...
router.get("/:id", verifyJWT, requireRole("admin"), async (req, res) => {
  ...
  try {
    const result = await pool.query(`SELECT id, username, email, role, profile, is_active,
...
router.patch("/:id/username", verifyJWT, [param("id").isUUID(), body("username").isString().trim().isLength({
  min: 2,
  max: 50
})], async (req, res) => {
  const errors = validationResult(req);
...
router.delete("/:id/mfa", verifyJWT, requireRole("admin"), [param("id").isUUID()], async (req, res) => {
  ...
  try {
    const existing = await pool.query(`SELECT id, COALESCE(mfa_enabled, false) AS mfa_enabled,
    ...
    await pool.query(`UPDATE v_b_users SET mfa_enabled = false, mfa_secret = NULL WHERE id = $1`, [id]);
    res.json({
      success: true,
      mfa_enabled: false,
      mfa_pending_setup: false
    });
  } catch (err) {
    console.error("DELETE /users/:id/mfa", err);
    res.status(500).json({
      error: "Error resetting MFA"
    });
  }
});
router.patch("/:id/email", verifyJWT, [param("id").isUUID(), body("email").isEmail()], async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) return res.status(400).json({
    errors: errors.array()
  });
  ...
  try {
    const target = await pool.query(`SELECT id, role, contact_id FROM v_b_users WHERE id = $1`, [id]);
    if (target.rows.length === 0) {
      return res.status(404).json({
        error: "User not found."
      });
