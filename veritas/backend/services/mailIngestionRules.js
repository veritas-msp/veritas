...
export function normalizeExclusionFilterRoot(rule = {}) {
  if (rule?.filterRoot?.type === "group") {
    return normalizeGroupNode(rule.filterRoot, {
      isRoot: true
    });
  }
...
export function buildMailContextFromEnvelope(message = {}) {
  ...
  const toAddresses = (Array.isArray(message?.envelope?.to) ? message.envelope.to : []).map(item => String(item?.address || "").trim()).filter(Boolean).join(", ");
  const ccAddresses = (Array.isArray(message?.envelope?.cc) ? message.envelope.cc : []).map(item => String(item?.address || "").trim()).filter(Boolean).join(", ");
  const replyToAddress = String((Array.isArray(message?.envelope?.replyTo) ? message.envelope.replyTo[0]?.address : "") || "").trim();
...
export function mailMatchesFilterRoot(filterRoot, context = {}) {
  const root = normalizeExclusionFilterRoot({
    filterRoot
  });
...
export function normalizeExclusionRule(row, idx = 0) {
  const legacyCriteria = Array.isArray(row?.criteria) ? row.criteria : row?.type || row?.value ? [{
    ...
    field: String(row?.type || "title_contains").trim() === "requester_email" ? "fromAddress" : String(row?.type || "title_contains").trim() === "field_contains" ? "body" : "subject",
    operator: "contains",
