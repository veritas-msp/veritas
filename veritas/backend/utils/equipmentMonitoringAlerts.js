...
function mapAlertRow(row) {
  ...
  return {
    ...
    lastKnownCriteria: row.last_known_criteria && typeof row.last_known_criteria === "object" ? row.last_known_criteria : {},
...
export async function getEquipmentAlertSettings(clientId, equipmentId, equipmentFamily) {
  const result = await pool.query(`SELECT * FROM v_b_equipment_monitoring_alerts
     WHERE client_id = $1 AND equipment_id = $2::uuid AND equipment_family = $3
     LIMIT 1`, [clientId, equipmentId, equipmentFamily]);
...
export async function setEquipmentAlertsEnabled({
  ...
  const result = await pool.query(`INSERT INTO v_b_equipment_monitoring_alerts
     ...
     ON CONFLICT (client_id, equipment_id, equipment_family) DO UPDATE SET
       ...
       updated_at = NOW()
     RETURNING *`, [clientId, equipmentId, equipmentFamily, equipmentName || null, Boolean(alertsEnabled)]);
...
export async function upsertEquipmentAlertSuspension({
  ...
  const normalizedType = suspensionType === "permanent" || suspensionType === "temporary" ? suspensionType : null;
  const result = await pool.query(`INSERT INTO v_b_equipment_monitoring_alerts
     ...
     ON CONFLICT (client_id, equipment_id, equipment_family) DO UPDATE SET
       ...
       updated_at = NOW()
     RETURNING *`, [clientId, equipmentId, equipmentFamily, equipmentName || null, normalizedType, normalizedType === "temporary" ? suspendedUntil : null, normalizedType ? new Date().toISOString() : null, suspendedBy, suspensionReason || null]);
  return mapAlertRow(result.rows[0]);
}
export async function clearEquipmentAlertSuspension(clientId, equipmentId, equipmentFamily) {
  const result = await pool.query(`UPDATE v_b_equipment_monitoring_alerts
...
export async function touchEquipmentAlertState({
  ...
  const criteriaJson = lastKnownCriteria && typeof lastKnownCriteria === "object" ? JSON.stringify(lastKnownCriteria) : null;
  const result = criteriaJson ? await pool.query(`INSERT INTO v_b_equipment_monitoring_alerts
         ...
         ON CONFLICT (client_id, equipment_id, equipment_family) DO UPDATE SET
           ...
           updated_at = NOW()
         RETURNING *`, [clientId, equipmentId, equipmentFamily, equipmentName || null, monitorStatus || "ok", criteriaJson, ticketId, alertAt]) : await pool.query(`INSERT INTO v_b_equipment_monitoring_alerts
         ...
         ON CONFLICT (client_id, equipment_id, equipment_family) DO UPDATE SET
           ...
           last_alert_at = COALESCE(EXCLUDED.last_alert_at, v_b_equipment_monitoring_alerts.last_alert_at),
           updated_at = NOW()
         RETURNING *`, [clientId, equipmentId, equipmentFamily, equipmentName || null, monitorStatus || "ok", ticketId, alertAt]);
  return mapAlertRow(result.rows[0]);
}
