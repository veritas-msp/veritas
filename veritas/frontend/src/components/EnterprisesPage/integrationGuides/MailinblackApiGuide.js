...
export default function MailinblackApiGuide({
  ...
  return <>
      <div className={formStyles.sectionHead}>
        <h3 className={formStyles.sectionTitle}>Get Client ID and Auth key</h3>
        <p className={formStyles.sectionDesc}>
          {dedicated ? "Generate an API key in the client's Control Center, then paste Client ID + Auth key into Veritas." : "Generate an API key in the partner portal, then paste Client ID + Auth key into Veritas."}
        </p>
      </div>
      ...
      <ol className={styles.guideSteps}>
        ...
        <li className={styles.guideStep}>
          ...
          <div className={styles.guideStepBody}>
            ...
            <p className={styles.guideStepDesc}>
              ...
              <strong>Protect</strong>, then copy both the <strong>Client ID</strong> and the{" "}
        ...
        <li className={styles.guideStep}>
          ...
          <div className={styles.guideStepBody}>
            ...
            <p className={styles.guideStepDesc}>
              API URL:{" "}
              <code className={styles.guideCode}>{dedicated ? "https://app.mailinblack.com/mibc-XX-YY" : DEFAULT_API_URL}</code>
              {dedicated ? " (racine du tenant client — région + n° d'instance, ex. mibc-fr-08 ou mibc-en-03, sans /admin/integration)." : " (Veritas route via la gateway partenaire /api)."} Enter{" "}
