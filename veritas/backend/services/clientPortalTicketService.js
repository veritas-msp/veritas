...
const ALLOWED_ATTACHMENT_EXTENSIONS = new Set([".pdf", ".jpg", ".jpeg", ".png", ".doc", ".docx", ".csv", ".xls", ".xlsx"]);
export const portalAttachmentUpload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    ...
    if (!ALLOWED_ATTACHMENT_EXTENSIONS.has(ext)) {
      return cb(new Error("File type not allowed. Accepted formats: PDF, JPG, PNG, DOC, DOCX, CSV, XLS, XLSX."));
    }
    cb(null, true);
  }
});
const SYSTEM_COMMENT_PREFIXES = ["[Linked ticket]", "[Split ticket]", "[Linked equipment]", "[Side conversation]", "[Resolution]", "[Resolution auto-close]", "[Resolution auto-clôture]", "[Resolution client]", "[Resolution client accepted]", "[Resolution client acceptée]"];
...
function buildPortalTicketDescription(baseDescription, {
  ...
  if (attempted) {
    parts.push("", "--- Actions already attempted ---", attempted);
  }
  const natureLabels = {
    hardware: "Hardware",
    software: "Software",
  ...
  if (natureKey && natureKey !== "unsure") {
    parts.push("", "--- Issue type ---", natureLabels[natureKey] || natureKey);
  }
  return parts.filter((line, index) => line !== "" || index === 0).join("\n").trim() || null;
}
function buildLinkedEquipmentComment(equipment, clientId) {
  const safeName = String(equipment?.name || equipment?.model || "Equipment").replace(/[\\\]]/g, "");
  const safeType = String(equipment?.type || "").replace(/[\\\]]/g, "");
  const safeClientId = String(clientId || "").replace(/[\\\]]/g, "");
  return `[Linked equipment] [event:added] [equipment_id:${equipment.id}] [name:${safeName}] [type:${safeType}] ` + `[client_id:${safeClientId}] [warranty:] [licenses:]`;
}
...
function mapPortalComment(row, attachments = [], authorProfile = null) {
  const authorLabel = authorProfile && typeof authorProfile === "object" ? authorProfile.display_name : authorProfile;
  return {
    id: row.id,
    content: row.content,
    is_internal: false,
    ...
    author_avatar: authorProfile && typeof authorProfile === "object" ? authorProfile.avatar || null : null,
...
export async function listPortalTickets(clientId, {
  status,
  search,
  limit = 50,
  offset = 0,
  ...
  const validationJoin = hasValidationTable ? `LEFT JOIN LATERAL (
...
export async function getPortalTicketDetail(clientId, ticketId, scope = {}) {
  ...
  return {
    ...
    comments: publicComments.map(row => mapPortalComment(row, attachmentsByComment.get(String(row.id)) || [], authorMap.get(String(row.author_user_id)) || null)),
    satisfaction,
    resolutionValidation
  };
}
export async function createPortalTicket({
  ...
  const fullDescription = buildPortalTicketDescription(description, {
    attemptedActions,
    issueNature
  });
...
export async function addPortalTicketComment({
  ...
  const result = await pool.query(`INSERT INTO v_b_ticket_comments (ticket_id, author_user_id, content, is_internal, created_at)
     ...
     RETURNING *`, [ticketId, userId || null, trimmed]);
  const createdComment = result.rows[0];
  const savedAttachments = [];
  for (const file of files) {
    const relativePath = `/uploads/tickets/${path.basename(file.path)}`;
    const attachmentResult = await pool.query(`INSERT INTO v_b_ticket_attachments
        (ticket_id, comment_id, uploaded_by, file_name, file_path, mime_type, file_size, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, NOW())
       RETURNING *`, [ticketId, createdComment.id, userId || null, file.originalname || path.basename(file.path), relativePath, file.mimetype || "application/octet-stream", Number(file.size || 0)]);
...
export async function updatePortalTicketComment({
  ...
  const trimmed = String(content || "").trim();
  const commentResult = await pool.query(`SELECT id, ticket_id, author_user_id, content, is_internal, created_at, updated_at
     FROM v_b_ticket_comments
     WHERE id = $1 AND ticket_id = $2 AND is_internal = false`, [commentId, ticketId]);
  ...
  const attachmentsResult = await pool.query(`SELECT id, ticket_id, comment_id, file_name, file_path, mime_type, file_size, created_at
     ...
     ORDER BY created_at ASC`, [commentId, ticketId]);
  ...
  const updateResult = await pool.query(`UPDATE v_b_ticket_comments
     ...
     RETURNING id, ticket_id, author_user_id, content, is_internal, created_at, updated_at`, [trimmed, commentId, ticketId]);
  await pool.query("UPDATE v_b_tickets SET updated_at = NOW() WHERE id = $1", [ticketId]);
  const authorMap = await resolveAuthorProfiles([userId]);
  return mapPortalComment(updateResult.rows[0], savedAttachments, authorMap.get(String(userId)) || {
    display_name: "You",
    avatar: null
  });
}
