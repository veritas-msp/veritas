...
import { buildDefaultOptionsObject, DEFAULT_CONTRACT_MODULES, listContractModuleOptions } from '../../utils/contractModuleOptions.js';
...
const HOME_EVENTS_UPCOMING_DAYS = 7;
const HOME_EVENTS_LIST_LIMIT = 50;
...
router.get("/clients", async (req, res) => {
  try {
    ...
    for (const client of clients) {
      ...
      let equipements = {};
      try {
        const serversResult = await pool.query(`SELECT data FROM v_b_clients_m_servers WHERE client_id = $1 AND data IS NOT NULL`, [id]);
        equipements.Serveurs = serversResult.rows.map(row => row.data);
        const nasResult = await pool.query(`SELECT data FROM v_b_clients_m_stockage WHERE client_id = $1 AND data IS NOT NULL`, [id]);
        equipements.NAS = nasResult.rows.map(row => row.data);
        const nddResult = await pool.query(`SELECT data FROM v_b_clients_m_ndd WHERE client_id = $1 AND data IS NOT NULL`, [id]);
        equipements.NDD = nddResult.rows.map(row => row.data);
        const antispamResult = await pool.query(`SELECT data FROM v_b_clients_m_antispam WHERE client_id = $1 AND data IS NOT NULL LIMIT 1`, [id]);
        if (antispamResult.rows.length > 0) {
          equipements.Antispam = antispamResult.rows[0].data;
        }
        const antivirusResult = await pool.query(`SELECT data FROM v_b_clients_m_antivirus WHERE client_id = $1 AND data IS NOT NULL LIMIT 1`, [id]);
        if (antivirusResult.rows.length > 0) {
          equipements.Antivirus = antivirusResult.rows[0].data;
        }
        const saveResult = await pool.query(`SELECT item_key, data FROM v_b_clients_m_save WHERE client_id = $1 AND data IS NOT NULL`, [id]);
        ...
        const o365Result = await pool.query(`SELECT data FROM v_b_clients_m_o365 WHERE client_id = $1 AND data IS NOT NULL LIMIT 1`, [id]);
    ...
    res.status(500).json({
      error: "Error during calcul statistics clients"
    });
  }
});
router.get('/reports', async (req, res) => {
  try {
    ...
    const timeSavedPerReport = 227.5;
    const totalTimeSaved = stats.total_reports * timeSavedPerReport;
    const totalTimeSavedHours = Math.round(totalTimeSaved / 60 * 100) / 100;
    const hourlyRate = 242.80 / 8;
...
async function safeContractModuleOptions() {
  try {
    return await listContractModuleOptions({
...
const LICENSE_MODULE_TABLES = ["v_b_clients_m_antivirus", "v_b_clients_m_antispam", "v_b_clients_m_save", "v_b_clients_m_o365", "v_b_clients_m_licences", "v_b_clients_m_ndd", "v_b_clients_m_ssl", "v_b_clients_m_firewall", "v_b_clients_m_toip"];
...
function resolveLicenseExpiration(entry = {}) {
  return entry.expiration || entry.expirityDate || entry.expirationDate || entry.expiryDate || entry.endDate || entry.validTo || entry.notAfter || entry.syncData?.license?.expirationDate || null;
}
...
function collectCyberLicenseAlertItems(parsed, ctx, alerts, seen) {
  const rootLabel = resolveLicenseItemLabel(parsed.logiciel, parsed.solution, parsed.solutionName, parsed.nom, parsed.name, parsed.provider, parsed.product, ctx.itemName);
  pushLicenseAlert(alerts, seen, ctx, rootLabel, resolveLicenseExpiration(parsed) || parsed.syncData?.license?.expirationDate);
  for (const collection of [parsed.solutions, parsed.licences, parsed.instances, parsed.items]) {
    if (!Array.isArray(collection)) continue;
    for (const entry of collection) {
      pushLicenseAlert(alerts, seen, ctx, resolveLicenseItemLabel(entry.logiciel, entry.solution, entry.nom, entry.name, entry.product, entry.label, entry.type, entry.jobName, entry.instance), resolveLicenseExpiration(entry));
    }
  }
}
function collectDomainLicenseAlertItems(parsed, ctx, alerts, seen) {
  pushLicenseAlert(alerts, seen, ctx, resolveLicenseItemLabel(parsed.nom, parsed.name, parsed.domaine, parsed.domain, parsed.hostname, parsed.host, ctx.itemName), resolveLicenseExpiration(parsed));
  for (const collection of [parsed.NDD, parsed.domains, parsed.domaines]) {
    if (!Array.isArray(collection)) continue;
    for (const entry of collection) {
      pushLicenseAlert(alerts, seen, ctx, resolveLicenseItemLabel(entry.nom, entry.name, entry.domaine, entry.domain), resolveLicenseExpiration(entry));
    }
  }
}
function collectFirewallLicenseAlertItems(parsed, ctx, alerts, seen) {
  ...
  for (const licence of parsed.licences) {
    pushLicenseAlert(alerts, seen, ctx, resolveLicenseItemLabel(licence.nom, licence.name, licence.type, licence.label), resolveLicenseExpiration(licence));
  }
}
...
async function fetchLicenseAlerts() {
  ...
  for (const table of LICENSE_MODULE_TABLES) {
    ...
    if (!meta) continue;
    try {
      const result = await pool.query(`SELECT m.client_id, m.data, m.item_key, c.id AS client_ref_id, c.name AS client_name
         FROM ${table} m
...
async function fetchHomeTicketsForUser({
  hasMajorIncident,
  hasSlaInfo,
  userId,
  hasTicketAssignees
  ...
  const sql = `SELECT t.id, t.ticket_number, t.title, t.description, t.status, t.priority, t.type, t.category, t.client_id, t.updated_at, t.created_at,
...
async function fetchPendingValidationTicketsForUser(userId) {
  if (!userId) return { rows: [] };
  try {
    const result = await pool.query(
      `SELECT t.id, t.type, t.category, t.status
         FROM v_b_tickets t
         INNER JOIN v_b_ticket_validation_requests vr
           ON vr.ticket_id = t.id
          AND vr.status = 'pending'
          AND vr.validator_user_id = $1::uuid
    ...
    // Table or soft-delete columns may be missing before migration.
...
function summarizePendingValidationByFamily(rows) {
  let support = 0;
  let sales = 0;
  for (const row of rows || []) {
    if (isSalesTicket(row?.type, row?.category)) sales += 1;
    else support += 1;
  }
  return {
    support: { toValidate: support },
    sales: { toValidate: sales }
  };
}
function mergeAssignedTicketStatsWithValidation(assignedStats, validationStats) {
  const base = assignedStats || {
    support: { new: 0, pending: 0, inProgress: 0, total: 0 },
    sales: { new: 0, pending: 0, inProgress: 0, total: 0 }
  };
  return {
    support: {
      ...base.support,
      toValidate: Number(validationStats?.support?.toValidate) || 0
    },
    sales: {
      ...base.sales,
      toValidate: Number(validationStats?.sales?.toValidate) || 0
    }
  };
}
...
function filterHomeEventsForUser(rows, userId, limit = HOME_LIST_LIMIT) {
  return rows.filter(row => isEventAssignedToUser(row, userId)).slice(0, limit).map(e => {
    const meta = parseEventDescriptionMeta(e.description);
    const scheduleAllDay = meta?.schedule?.allDay;
    return {
      id: e.id,
      title: e.title,
      type: e.type,
      ...
      allDay: typeof scheduleAllDay === "boolean" ? scheduleAllDay : null,
      clientId: e.client_id,
      clientName: e.client_name
    };
  });
}
function rankAndSliceHomeTickets(rows, {
  hasSlaInfo,
  slaSettings,
...
function summarizeAssignedHomeTickets(rows) {
  ...
  return {
    ...
    pending,
    inProgress,
    total: neu + pending + inProgress
  };
}
function summarizeAssignedHomeTicketsByFamily(rows) {
  const supportRows = [];
  const salesRows = [];
  for (const row of rows || []) {
    if (isSalesTicket(row?.type, row?.category)) salesRows.push(row);
    else supportRows.push(row);
  }
  return {
    support: summarizeAssignedHomeTickets(supportRows),
    sales: summarizeAssignedHomeTickets(salesRows)
  };
}
router.get("/home-dashboard", verifyJWT, async (req, res) => {
  try {
    const userId = req.user?.id || null;
    const [hasMajorIncident, hasSlaInfo, hasTicketAssignees, slaSettings, monitorableEquipment] = await Promise.all([hasTicketColumn("is_major_incident"), hasTicketColumn("sla_info"), hasTicketAssigneesTable(), loadSlaSettings().catch(err => {
      console.error("home-dashboard slaSettings", err);
      return null;
    }), safeMonitorableEquipmentStats()]);
    const [clientsUnderContract, campaigns, antispamConfigs, antivirusConfigs, reportsGenerated, tenants, domains, activeAgents, rmmAgents, clientsRows, ticketStatusRows, recentTicketRows, pendingValidationRows, eventRows, orgSettingsRows] = await Promise.all([countOrZero(`SELECT COUNT(*)::int AS count FROM v_b_clients`), countOrZero(`SELECT COUNT(*)::int AS count FROM v_b_clients_c_campaign`), countOrZero(`SELECT COUNT(*)::int AS count FROM v_b_clients_m_antispam WHERE data IS NOT NULL`), countOrZero(`SELECT COUNT(*)::int AS count FROM v_b_clients_m_antivirus WHERE data IS NOT NULL`), countOrZero(`SELECT COUNT(*)::int AS count FROM document_history`), countOrZero(`SELECT COUNT(*)::int AS count FROM v_b_clients_azure WHERE tenant_id IS NOT NULL AND btrim(tenant_id::text) <> ''`), countOrZero(`SELECT COUNT(*)::int AS count FROM v_b_clients_m_ndd WHERE data IS NOT NULL`), countOrZero(ACTIVE_AGENTS_COUNT_SQL), countOrZero(RMM_AGENTS_COUNT_SQL), pool.query(`SELECT id, name, contrat, options FROM v_b_clients ORDER BY name ASC`).catch(() => ({
      rows: []
    })), pool.query(`SELECT status, COUNT(*)::int AS count
           FROM v_b_tickets
           GROUP BY status`).catch(() => ({
      rows: []
    })), fetchHomeTicketsForUser({
      hasMajorIncident,
      hasSlaInfo,
      userId,
      hasTicketAssignees
      ...
      rows: []
    })), fetchPendingValidationTicketsForUser(userId).catch(() => ({
      rows: []
    })), pool.query(`SELECT e.id, e.title, e.type, e.start, e."end", e.client_id, e.description, e.assigned_user_id, c.name AS client_name
           FROM v_b_events e
           LEFT JOIN v_b_clients c ON c.id = e.client_id
           WHERE e."end" >= NOW() - INTERVAL '12 hours'
             AND e.start < NOW() + INTERVAL '${HOME_EVENTS_UPCOMING_DAYS} days'
           ORDER BY e.start ASC
           LIMIT ${HOME_EVENTS_POOL_LIMIT}`).catch(() => ({
      rows: []
    })), pool.query(`SELECT value FROM v_b_settings WHERE section = 'general' AND key = 'app_organization_name' LIMIT 1`).catch(() => ({
      rows: []
    }))]);
    let backupJobs = 0;
    try {
      const saveR = await pool.query(`SELECT item_key, data FROM v_b_clients_m_save WHERE data IS NOT NULL`);
    ...
    try {
      const urgentR = await pool.query(`SELECT COUNT(*)::int AS count FROM v_b_tickets
    ...
    const organizationName = String(orgSettingsRows.rows[0]?.value || "").trim() || "Veritas";
    ...
    const fullDashboard = {
      ...
      ticketsByStatus,
      assignedTicketStats: mergeAssignedTicketStatsWithValidation(
        summarizeAssignedHomeTicketsByFamily(recentTicketRows.rows),
        summarizePendingValidationByFamily(pendingValidationRows.rows)
      ),
      ...
      contractAlerts,
      licenseAlerts,
      upcomingEvents: filterHomeEventsForUser(eventRows.rows, userId, HOME_EVENTS_LIST_LIMIT),
      infrastructure: {
        families: equipmentFamilies,
        equipMonitoredTotal,
        equipUnderSurveillanceCount,
...
router.get("/home-kpis", async (req, res) => {
  if (isCommunity()) {
    return res.status(403).json({
      error: "Advanced indicators reserved for Veritas Pro",
      code: "PRO_FEATURE_REQUIRED"
    });
  }
  try {
    ...
    const [clientsUnderContract, campaigns, antispamConfigs, antivirusConfigs, reportsGenerated, tenants, domains] = await Promise.all([countOrZero(`SELECT COUNT(*)::int AS count FROM v_b_clients`), countOrZero(`SELECT COUNT(*)::int AS count FROM v_b_clients_c_campaign`), countOrZero(`SELECT COUNT(*)::int AS count FROM v_b_clients_m_antispam WHERE data IS NOT NULL`), countOrZero(`SELECT COUNT(*)::int AS count FROM v_b_clients_m_antivirus WHERE data IS NOT NULL`), countOrZero(`SELECT COUNT(*)::int AS count FROM document_history`), countOrZero(`SELECT COUNT(*)::int AS count FROM v_b_clients_azure WHERE tenant_id IS NOT NULL AND btrim(tenant_id::text) <> ''`), countOrZero(`SELECT COUNT(*)::int AS count FROM v_b_clients_m_ndd WHERE data IS NOT NULL`)]);
    let backupJobs = 0;
    try {
      const saveR = await pool.query(`SELECT item_key, data FROM v_b_clients_m_save WHERE data IS NOT NULL`);
      backupJobs = countAllBackupJobsFromSaveRows(saveR.rows);
      ...
      try {
        const saveR = await pool.query(`SELECT data FROM v_b_clients_m_save WHERE data IS NOT NULL`);
        backupJobs = countAllBackupJobsFromSaveRows(saveR.rows.map(r => ({
          item_key: null,
          data: r.data
        })));
      } catch {
