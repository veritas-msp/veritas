...
import { getAllEnabledFeeds } from "./techNewsFeedsConfig.js";
const CACHE_TTL_MS = 15 * 60 * 1000;
const FETCH_TIMEOUT_MS = 12_000;
const MAX_ITEMS_PER_FEED = 8;
const MAX_TOTAL_ITEMS = 28;
/** Max plain-text body kept from RSS (content:encoded / description). */
...
function extractBestBody(block) {
  const candidates = [
  ...
  return candidates[0].slice(0, MAX_BODY_CHARS);
}
...
function parseFeedXml(xml, feedMeta) {
  ...
  for (const block of blocks.slice(0, MAX_ITEMS_PER_FEED)) {
    ...
    const body = extractBestBody(block);
...
async function loadAllFeeds() {
  const feeds = await getAllEnabledFeeds();
...
export async function getTechNewsFeed(_localeInput) {
  const now = Date.now();
