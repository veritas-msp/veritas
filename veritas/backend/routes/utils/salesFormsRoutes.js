...
const FIELD_TYPES = new Set(["text", "textarea", "select", "checkbox", "user", "contact", "client", "number", "date"]);
...
async function loadUsageCountsByFormIds(formIds = []) {
  ...
  try {
    const result = await pool.query(`SELECT sales_form_data->>'formId' AS form_id,
            COUNT(*)::int AS usage_count
       FROM v_b_tickets
      WHERE sales_form_data->>'formId' = ANY($1::text[])
      GROUP BY sales_form_data->>'formId'`, [formIds]);
    const counts = {};
    ...
    console.warn("Failed to load sales form usage counts:", err.message);
    return {};
  }
}
...
router.get("/", verifyJWT, [query("kind").optional().isIn(["prestation", "installation"]), query("includeDisabled").optional().isBoolean()], async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      error: "Error during loading saforms"
    });
  }
});
...
router.post("/", verifyJWT, [body("kind").isIn(["prestation", "installation"]), body("key").isString().notEmpty(), body("label").isString().notEmpty(), body("icon").optional().isString(), body("categorySlug").optional().isString(), body("description").optional().isString(), body("displayOrder").optional().isInt(), body("enabled").optional().isBoolean(), body("visibility").optional().isIn(["public", "assigned"]), body("profileNames").optional().isArray(), body("userIds").optional().isArray(), body("teamIds").optional().isArray(), body("ticketTargets").optional().isObject()], async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(`INSERT INTO v_b_sales_form_definitions
         ...
         RETURNING *`, [id, kind, formKey, label, icon, categorySlug, description, displayOrder, enabled, visibility, JSON.stringify(ticketTargets)]);
    ...
    if (String(err?.code) === "23505") {
      return res.status(409).json({
        error: "A form with this key or category already exists"
      });
    }
...
router.put("/:formId", verifyJWT, [param("formId").isString().notEmpty(), body("kind").optional().isIn(["prestation", "installation"]), body("key").optional().isString(), body("label").optional().isString(), body("icon").optional().isString(), body("categorySlug").optional().isString(), body("description").optional().isString(), body("displayOrder").optional().isInt(), body("enabled").optional().isBoolean(), body("visibility").optional().isIn(["public", "assigned"]), body("profileNames").optional().isArray(), body("userIds").optional().isArray(), body("teamIds").optional().isArray(), body("ticketTargets").optional().isObject()], async (req, res) => {
  if (!isAdminUser(req)) return res.status(403).json({
    error: "Access restricted to administrators"
  });
  const validationResponse = validationErrorOrNull(req, res);
  ...
  try {
    ...
    if (updates.length > 0) {
      ...
      values.push(formId);
      await pool.query(`UPDATE v_b_sales_form_definitions SET ${updates.join(", ")} WHERE id = $${idx}`, values);
    }
    ...
    if (String(err?.code) === "23505") {
      return res.status(409).json({
        error: "Conflict of key or category"
      });
    }
...
router.post("/:formId/fields", verifyJWT, [param("formId").isString().notEmpty(), body("fieldKey").isString().notEmpty(), body("label").isString().notEmpty(), body("fieldType").optional().isString(), body("required").optional().isBoolean(), body("placeholder").optional().isString(), body("options").optional().isArray(), body("visibilityRules").optional().isObject(), body("displayOrder").optional().isInt(), body("enabled").optional().isBoolean()], async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(`INSERT INTO v_b_sales_form_fields
...
router.put("/:formId/fields/:fieldId", verifyJWT, [param("formId").isString().notEmpty(), param("fieldId").isString().notEmpty(), body("fieldKey").optional().isString(), body("label").optional().isString(), body("fieldType").optional().isString(), body("required").optional().isBoolean(), body("placeholder").optional().isString(), body("options").optional().isArray(), body("visibilityRules").optional().isObject(), body("displayOrder").optional().isInt(), body("enabled").optional().isBoolean()], async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(`UPDATE v_b_sales_form_fields
         SET ${updates.join(", ")}
...
router.delete("/:formId/fields/:fieldId", verifyJWT, [param("formId").isString().notEmpty(), param("fieldId").isString().notEmpty()], async (req, res) => {
  ...
  if (validationResponse) return;
  try {
    const result = await pool.query(`DELETE FROM v_b_sales_form_fields WHERE id = $1 AND form_id = $2`, [String(req.params.fieldId), String(req.params.formId)]);
    if (result.rowCount === 0) return res.status(404).json({
      error: "Field not found"
    });
    return res.json({
