...
router.use((req, res, next) => {
  const isMigrate = req.path === "/migrate" || req.path.startsWith("/migrations/");
...
router.post("/env", requireSetupIncomplete, [body("jwtSecret").optional().isString().isLength({
  min: 16
}), body("encryptionKey").optional().isString().isLength({
  min: 16
}), body("allowedOrigins").optional().isString(), body("frontendBaseUrl").optional().isString(), body("apiBaseUrl").optional().isString(), body("port").optional().isInt({
  min: 1,
  max: 65535
}), body("edition").optional().isIn(["community", "pro"])], async (req, res) => {
  ...
  const apiBaseUrl = req.body.apiBaseUrl || (req.body.port ? `http://localhost:${req.body.port}` : "http://localhost:3001");
...
router.post("/database", requireSetupIncomplete, [body("db_host").isString().trim().notEmpty(), body("db_port").isInt({
  min: 1,
  max: 65535
}), body("db_user").isString().trim().notEmpty(), body("db_password").isString(), body("db_name").isString().trim().notEmpty()], async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(400).json({
      code: "SETUP_VALIDATION_FAILED",
...
router.post("/migrate", requireSetupIncomplete, async (req, res) => {
  try {
    const stepByStep = req.body?.stepByStep === true || req.query.step === "1" || req.query.stepByStep === "1";
    if (stepByStep) {
...
async function getSetupAdminUser() {
  const {
    rows
  } = await pool.query(`SELECT id, email, COALESCE(mfa_enabled, false) AS mfa_enabled, mfa_secret
     FROM v_b_users
     WHERE role = 'admin' AND is_active = TRUE
     LIMIT 1`);
  return rows[0] || null;
}
router.post("/admin", requireSetupIncomplete, [body("email").isEmail(), body("password").isString(), body("username").optional().isString().trim().isLength({
  min: 2,
  max: 50
})], async (req, res) => {
  const errors = validationResult(req);
  ...
  try {
    const existing = await pool.query(`SELECT 1 FROM v_b_users WHERE role = 'admin' LIMIT 1`);
    ...
    const id = randomUUID();
    await pool.query(`INSERT INTO v_b_users (id, email, username, password_hash, role, profile, is_active)
         VALUES ($1, $2, $3, $4, 'admin', 'Super Admin', TRUE)`, [id, normalizedEmail, username || normalizedEmail.split("@")[0], hash]);
    res.json({
      success: true,
...
router.post("/admin/mfa/verify", requireSetupIncomplete, [body("code").isString().trim().isLength({
  min: 6,
  max: 8
})], async (req, res) => {
  const errors = validationResult(req);
