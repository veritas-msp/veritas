...
const VIDEO_SURVEILLANCE_TABLES = ["v_b_clients_m_videosurveillance", "v_b_clients_m_camera", "v_b_clients_m_cameras"];
...
async function countFamilyWithMonitoring({
  table,
  where,
  monitoredWhere = CHECKMK_MONITORED_WHERE
}) {
  const count = await countFamilyTable({
    table,
    where
  });
...
async function countFamilyTable({
  table,
  where
}) {
...
async function countCustomFamilyMonitoring() {
  try {
    ...
    `);
    return new Map(result.rows.map(row => [String(row.family_key), {
...
async function countCustomFamilies() {
  try {
    ...
    const definitions = await listEquipmentFamilies({
