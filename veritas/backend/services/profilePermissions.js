...
export async function resolveEffectiveProfile(name, visited = new Set()) {
  ...
  if (!key) return null;
  if (visited.has(key)) {
    throw new Error(`Inheritance cycle detected for profile “${key}”`);
  }
  visited.add(key);
  const row = await getProfileRow(key);
  if (!row) return null;
  if (!row.parent_profile) {
    return {
      ...row,
...
export async function profileHasChildren(name) {
  const result = await pool.query(`SELECT COUNT(*)::int AS total FROM v_b_users_profiles WHERE parent_profile = $1`, [name]);
