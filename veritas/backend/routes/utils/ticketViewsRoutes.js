...
import { BUILTIN_TICKET_VIEWS, BUILTIN_TICKET_VIEW_IDS, resolveBuiltinViewRules } from "../../utils/builtinTicketViews.js";
...
async function userCanManageViews(req, pageScope) {
  if (isAdminUser(req)) return true;
  return userHasAnyPermission(req.user, [manageViewsKeyForScope(pageScope)]);
}
...
async function canWriteView(view, req) {
  if (!view) return false;
  const canManage = await userCanManageViews(req, view.page_scope);
  if (!canManage) return false;
  if (view.is_builtin) return true;
  if (isAssignedVisibility(view.visibility) || view.visibility === "public") return true;
  return view.owner_user_id && String(view.owner_user_id) === String(req.user?.id);
}
async function canChangeViewVisibility(nextVisibility, view, req) {
  const normalizedNext = normalizeVisibility(nextVisibility);
  if (normalizedNext === "private") return true;
  return userCanManageViews(req, view?.page_scope || req.body?.pageScope);
}
router.get("/counts", verifyJWT, [query("pageScope").optional().isIn(["ticket", "ticket_sales"]), query("viewId").optional().isString(), query("search").optional().isString(), query("ticketType").optional().isString(), query("scope").optional().isIn(["all", "views", "statuses"])], async (req, res) => {
  ...
  try {
    ...
    const ticketFamily = pageScope === "ticket_sales" ? "sales" : "support";
    ...
    let views = {};
    if (scope === "all" || scope === "views") {
      const builtinViews = pageScope === "ticket_sales" ? BUILTIN_TICKET_VIEWS.filter(view => String(view.id) !== String(BUILTIN_TICKET_VIEW_IDS.MONITORING)) : BUILTIN_TICKET_VIEWS;
      const viewsToCount = [...builtinViews.map(view => ({
        id: view.id,
        rules: view.rules
      })), ...customViews];
...
router.get("/admin", verifyJWT, [query("pageScope").optional().isIn(["ticket", "ticket_sales"])], async (req, res) => {
  const pageScope = String(req.query.pageScope || "ticket");
  if (!(await userCanManageViews(req, pageScope))) {
    return res.status(403).json({
      error: "You do not have permission to manage views.",
...
router.post("/", verifyJWT, [body("name").isString().trim().isLength({
  ...
  if (validationErrorOrNull(req, res)) return;
  try {
    const pageScope = String(req.body.pageScope || "ticket");
    if (!(await userCanManageViews(req, pageScope))) {
      return res.status(403).json({
        error: "You do not have permission to create views.",
        code: "PERMISSION_DENIED"
      });
    }
    const visibility = normalizeVisibility(req.body.visibility || "private");
    if ((visibility === "public" || visibility === "assigned") && !(await userCanManageViews(req, pageScope))) {
      return res.status(403).json({
        error: "Only users with manage views permission can create an assigned or public view"
      });
    }
    ...
    const result = await pool.query(`INSERT INTO v_b_ticket_views
         ...
         RETURNING *`, [String(req.body.name).trim(), req.body.description ? String(req.body.description).trim() : null, pageScope, visibility, req.user?.id, String(req.body.icon || "mdi:view-list").trim(), req.body.color ? String(req.body.color).trim() : null, JSON.stringify(rules), String(req.body.sortBy || "updated_at"), String(req.body.sortDirection || "desc"), Number(req.body.displayOrder) || 0]);
...
router.put("/:viewId", verifyJWT, [param("viewId").isUUID(), ...viewBodyValidators], async (req, res) => {
  ...
  try {
    ...
    if (!(await canWriteView(view, req))) return res.status(403).json({
    ...
    if ((nextVisibility === "public" || nextVisibility === "assigned") && !(await canChangeViewVisibility(nextVisibility, view, req))) {
      return res.status(403).json({
        error: "Only users with manage views permission can publish a view"
      });
    }
