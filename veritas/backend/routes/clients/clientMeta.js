...
export function registerClientMetaRoutes(router) {
  router.get("/support-credits/packs", verifyJWT, requirePermission("clients_detail.credits"), async (req, res) => {
    try {
  ...
  router.get("/tags/catalog", verifyJWT, requirePermission("clients.view"), async (req, res) => {
    try {
  ...
  router.get("/:id/tags", verifyJWT, requirePermission("clients.view"), async (req, res) => {
    try {
  ...
  router.post("/:id/tags", verifyJWT, requirePermission("clients_detail.tags"), async (req, res) => {
    try {
      const clientId = await resolveClientId(req.params.id);
      if (!clientId) return res.status(404).json({
        error: "Client not found"
  ...
  router.delete("/:id/tags/:tagId", verifyJWT, requirePermission("clients_detail.tags"), async (req, res) => {
    try {
      const clientId = await resolveClientId(req.params.id);
      if (!clientId) return res.status(404).json({
        error: "Client not found"
  ...
  router.get("/:id/notes", verifyJWT, requirePermission("clients.view"), async (req, res) => {
    try {
      const clientId = await resolveClientId(req.params.id);
      if (!clientId) return res.status(404).json({
        error: "Client not found"
  ...
  router.post("/:id/notes", verifyJWT, requirePermission("clients_detail.notes"), async (req, res) => {
    try {
      const clientId = await resolveClientId(req.params.id);
      if (!clientId) return res.status(404).json({
        error: "Client not found"
  ...
  router.put("/:id/notes/:noteId", verifyJWT, requirePermission("clients_detail.notes"), async (req, res) => {
    try {
      const clientId = await resolveClientId(req.params.id);
      if (!clientId) return res.status(404).json({
        error: "Client not found"
  ...
  router.delete("/:id/notes/:noteId", verifyJWT, requirePermission("clients_detail.notes"), async (req, res) => {
    try {
      const clientId = await resolveClientId(req.params.id);
      if (!clientId) return res.status(404).json({
        error: "Client not found"
  ...
  router.get("/:id/support-credits", verifyJWT, requirePermission("clients.view"), async (req, res) => {
    try {
      const clientId = await resolveClientId(req.params.id);
      if (!clientId) return res.status(404).json({
        error: "Client not found"
  ...
  router.post("/:id/support-credits", verifyJWT, requirePermission("clients_detail.credits"), async (req, res) => {
    try {
      const clientId = await resolveClientId(req.params.id);
      if (!clientId) return res.status(404).json({
        error: "Client not found"
  ...
  router.put("/:id/support-credits/packs/:packId", verifyJWT, requirePermission("clients_detail.credits"), async (req, res) => {
    try {
      const clientId = await resolveClientId(req.params.id);
      if (!clientId) return res.status(404).json({
        error: "Client not found"
  ...
  router.delete("/:id/support-credits/packs/:packId", verifyJWT, requirePermission("clients_detail.credits"), async (req, res) => {
    try {
      const clientId = await resolveClientId(req.params.id);
      if (!clientId) return res.status(404).json({
        error: "Client not found"
