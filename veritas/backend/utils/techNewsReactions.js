...
export async function getReactionsForArticles(articleIds = [], currentUserId = null) {
  ...
  const result = await pool.query(`SELECT r.article_id, r.user_id, r.emoji, r.created_at, r.updated_at,
            u.username, u.email
...
export async function getArticleReactions(articleId, currentUserId = null) {
  ...
  if (!id) return {
    articleId: id,
    reactions: emptyArticleReactions(),
    mine: null
  };
  const result = await pool.query(`SELECT r.user_id, r.emoji, r.created_at, r.updated_at, u.username, u.email
     FROM v_b_tech_news_reactions r
     JOIN v_b_users u ON u.id = r.user_id
     WHERE r.article_id = $1
     ORDER BY r.updated_at ASC`, [id]);
  const reactions = buildArticleReactions(result.rows);
  const mineRow = currentUserId ? result.rows.find(row => row.user_id === currentUserId) : null;
  return {
    articleId: id,
    reactions,
...
export async function toggleTechNewsReaction(articleId, userId, emoji) {
  ...
  if (!ALLOWED_REACTION_EMOJIS.includes(normalizedEmoji)) {
    const err = new Error("Unauthorized emoji");
    err.status = 400;
    throw err;
  }
  const existing = await pool.query(`SELECT emoji FROM v_b_tech_news_reactions WHERE article_id = $1 AND user_id = $2`, [id, userId]);
  let action = "added";
  if (existing.rows[0]?.emoji === normalizedEmoji) {
    await pool.query(`DELETE FROM v_b_tech_news_reactions WHERE article_id = $1 AND user_id = $2`, [id, userId]);
    action = "removed";
  } else if (existing.rows[0]) {
    await pool.query(`UPDATE v_b_tech_news_reactions
       SET emoji = $1, updated_at = NOW()
       WHERE article_id = $2 AND user_id = $3`, [normalizedEmoji, id, userId]);
    action = "updated";
  } else {
    await pool.query(`INSERT INTO v_b_tech_news_reactions (article_id, user_id, emoji)
       VALUES ($1, $2, $3)`, [id, userId, normalizedEmoji]);
  }
