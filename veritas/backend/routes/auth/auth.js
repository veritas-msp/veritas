...
import { buildSessionPayload, clearImpersonatorCookie, clearSessionCookie, isImpersonationPayload, buildImpersonationClientPayload, buildImpersonationAgentPayload, setImpersonatorCookie, setSessionCookie, signSessionToken } from "../../utils/authSession.js";
...
router.post("/refresh", verifyJWT, async (req, res) => {
  try {
    ...
    if (isImpersonationPayload(req.user) && req.cookies?.impersonator_token) {
      ...
      if (req.user.purpose === "agent_impersonation" || String(user.role || "").toLowerCase() !== "client") {
        token = signSessionToken(buildImpersonationAgentPayload(user, impersonator));
      } else {
        token = signSessionToken(buildImpersonationClientPayload(user, impersonator));
      }
    } else {
...
router.post("/impersonate/users/:id", verifyJWT, async (req, res) => {
  try {
    if (isImpersonationPayload(req.user) || req.cookies?.impersonator_token) {
      return res.status(400).json({
    ...
    const {
      rows: actorRows
    } = await pool.query(`SELECT id, email, username, role, profile, COALESCE(is_active, true) AS is_active
       FROM v_b_users WHERE id = $1`, [req.user.id]);
    ...
    if (!isSuperAdminPresetProfile(actor.profile)) {
      return res.status(403).json({
        error: "Only Super Admin can impersonate agents.",
    ...
    const {
      rows: targetRows
       ...
       FROM v_b_users WHERE id = $1`, [targetId]);
    ...
    if (String(target.role || "").toLowerCase() === "client") {
      return res.status(400).json({
        error: "Use portal impersonation for client contacts."
      });
    }
    if (target.is_active === false) {
      return res.status(403).json({
        error: "This agent account is disabled."
      });
    }
    ...
    const sessionToken = signSessionToken(buildImpersonationAgentPayload(target, {
      id: actor.id,
      email: actor.email
    }));
    setImpersonatorCookie(req, res, agentToken);
    setSessionCookie(req, res, sessionToken);
    console.info(`[impersonation][agent][audit] ts=${new Date().toISOString()} ip=${req.ip} actor=${actor.id} actor_email=${actor.email} target=${target.id} target_email=${target.email}`);
    res.json({
      id: target.id,
      email: target.email,
    ...
    console.error("POST /auth/impersonate/users/:id:", err);
    res.status(500).json({
