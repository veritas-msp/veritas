...
export function cookieOptions(req) {
  const secure = req.protocol === "https" || req.headers["x-forwarded-proto"] === "https" || req.secure === true;
  return {
    httpOnly: true,
    secure,
...
export function isImpersonationPayload(user) {
  return Boolean(user?.impersonated_by || user?.purpose === "client_impersonation" || user?.purpose === "agent_impersonation");
}
...
export function buildImpersonationAgentPayload(targetUser, impersonator) {
  return {
    id: targetUser.id,
    email: targetUser.email,
    role: targetUser.role,
    profile: targetUser.profile ?? null,
    client_id: targetUser.client_id ?? null,
    impersonated_by: impersonator.id,
    impersonated_by_email: impersonator.email ?? null,
    purpose: "agent_impersonation"
  };
}
