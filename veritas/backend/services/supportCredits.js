...
export function isSalesTicket(type, category) {
  ...
  if (SALES_TYPES.has(normalizedType)) return true;
...
export function resolveSalesTicketType(type, category, salesKind = null) {
  const kind = String(salesKind || "").trim().toLowerCase();
  if (kind === "prestation" || kind === "installation") return kind;
  const normalizedType = String(type || "").trim().toLowerCase();
  if (SALES_TYPES.has(normalizedType)) return normalizedType;
  const normalizedCategory = String(category || "");
  if (normalizedCategory.startsWith("installation-")) return "installation";
  if (normalizedCategory.startsWith("prestation-")) return "prestation";
  return null;
}
