...
export function resolveDatabaseCredentials(incoming = {}, current = {}) {
  const next = normalizeDatabaseConfig({
    ...current,
    ...incoming
  });
  ...
  for (const meta of DB_SETTING_META) {
    ...
    await pool.query(`INSERT INTO v_b_settings (key, value, label, section, value_encrypted, value_iv, value_auth_tag)
       ...
       ON CONFLICT (key) DO UPDATE SET
         ...
         value_auth_tag = EXCLUDED.value_auth_tag`, [meta.key, enc.value, meta.label, meta.section, enc.value_encrypted, enc.value_iv, enc.value_auth_tag]);
  }
  return normalized;
}
