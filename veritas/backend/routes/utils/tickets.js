...
import {
  ...
  getPrivateTicketTableColumnsSettingKey,
...
async function requireTicketCommentPermission(req, res, next) {
  try {
    ...
    const isInternal = parseBodyBoolean(req.body?.isInternal);
    if (!isInternal) {
      return requirePermission(isSales ? "sales_detail.public_reply" : "tickets_detail.public_reply")(req, res, next);
    }
    return requirePermission(isSales ? "sales.view" : "tickets.view")(req, res, next);
  } catch (err) {
    console.error("[permissions] Ticket comment check failed:", err.message);
    return res.status(500).json({
      error: "Permission check failed."
    });
  }
}
async function requireTicketCommentEditPermission(req, res, next) {
  try {
    const ticketResult = await pool.query("SELECT id, type, category FROM v_b_tickets WHERE id = $1", [req.params.id]);
    if (!ticketResult.rows[0]) {
      return res.status(404).json({
    ...
    const isSales = isSalesTicketRow(ticketResult.rows[0]);
    const removingAttachments = Array.isArray(req.body?.removeAttachmentIds) && req.body.removeAttachmentIds.length > 0 || Array.isArray(req.body?.removeAttachmentPaths) && req.body.removeAttachmentPaths.length > 0;
    if (removingAttachments) {
      const ok = await userHasAnyPermission(req.user, [isSales ? "sales_detail.delete_attachments" : "tickets_detail.delete_attachments", isSales ? "sales_detail.edit_messages" : "tickets_detail.edit_messages"]);
      if (!ok) {
    ...
    return requirePermission(isSales ? "sales_detail.edit_messages" : "tickets_detail.edit_messages")(req, res, next);
  } catch (err) {
    console.error("[permissions] Ticket comment edit check failed:", err.message);
    return res.status(500).json({
      error: "Permission check failed."
    });
  }
}
async function requireTicketStatusPermission(req, res, next) {
  try {
    const ticketResult = await pool.query("SELECT id, type, category FROM v_b_tickets WHERE id = $1", [req.params.id]);
    if (!ticketResult.rows[0]) {
      return res.status(404).json({
    ...
    const isSales = isSalesTicketRow(ticketResult.rows[0]);
    const status = normalizeIncomingStatus(req.body?.status);
    if (isTicketClosedStatus(status) || status === "resolved") {
      if (isSales) {
        return requireAnyPermission("sales_detail.public_reply", "sales_detail.report")(req, res, next);
      }
      return requireAnyPermission("tickets_detail.resolve", "tickets_detail.split", "tickets_detail.public_reply")(req, res, next);
    }
    return requirePermission(isSales ? "sales.view" : "tickets.view")(req, res, next);
  } catch (err) {
    console.error("[permissions] Ticket status check failed:", err.message);
    return res.status(500).json({
      error: "Permission check failed."
    });
  }
...
router.put("/automation-config", verifyJWT, [body("commentTemplates").optional().isArray(), body("macros").optional().isArray(), body("emailInboxes").optional().isArray(), body("exclusionRules").optional().isArray(), body("autoReplyRules").optional().custom(value => Array.isArray(value) || value && typeof value === "object"), body("autoReplyTemplate").optional().isString(), body("notificationSettings").optional().isObject(), body("mailCollectors").optional().isArray(), body("mailCollectSettings").optional().isObject()], async (req, res) => {
  ...
  try {
    const canManage = await userHasAnyPermission(req.user, ["tickets.manage"]);
    const canExclude = await userHasAnyPermission(req.user, ["tickets_detail.exclude"]);
    if (!canManage && !canExclude) {
      return res.status(403).json({
        error: "You do not have permission to perform this action.",
        code: "PERMISSION_DENIED"
      });
    }
    const current = await loadTicketAutomationRawConfig();
    const incoming = req.body || {};
    let payload = incoming;
    if (!canManage && canExclude) {
      // Exclude-only agents may only append/change exclusion rules.
      payload = {
        commentTemplates: current.commentTemplates,
        macros: current.macros,
        emailInboxes: current.emailInboxes,
...
router.post("/search", verifyJWT, requirePermission("tickets.view"), [body("viewRules").optional({
  nullable: true
}).custom(value => value == null || typeof value === "object"), body("viewMode").optional().isIn(["active", "trash"]), body("status").optional().isString(), body("ticketType").optional().isString(), body("ticketFamily").optional().isIn(["support", "sales", "all"]), body("search").optional().isString(), body("sortBy").optional().isString(), body("sortDirection").optional().isIn(["asc", "desc"]), body("limit").optional().isInt({
  min: 1,
...
router.post("/", verifyJWT, requirePermission("tickets.create"), [body("title").notEmpty().withMessage("Title is required"), body("description").optional().isString(), body("status").optional().isIn(STATUS_VALUES), body("priority").optional().isIn(PRIORITY_VALUES), body("type").optional().isString(), body("category").optional({
  ...
  try {
    ...
    const insertTicket = async (dbClient, rule = null) => {
      ...
      const values = [ticketTitle, ticketDescription, normalizedStatus, resolvedPriority, ticketType, ticketCategory, channel, resolvedClientId];
...
router.get("/table-columns", verifyJWT, async (req, res) => {
  try {
    ...
    const pageScope = normalizeTicketTableColumnsPageScope(req.query?.pageScope);
    const publicColumns = await loadPublicTicketTableColumns(pageScope);
    const privateColumns = await loadPrivateTicketTableColumns(req.user?.id, pageScope);
...
router.put("/table-columns/public", verifyJWT, async (req, res) => {
  try {
    const pageScope = normalizeTicketTableColumnsPageScope(req.body?.pageScope ?? req.query?.pageScope);
    const publicViewsKeyByScope = {
      ticket: "tickets.public_views",
      ticket_sales: "sales.public_views",
      enterprises: "clients.public_views",
      contacts: "contacts.public_views"
    };
    const publicViewsKey = publicViewsKeyByScope[pageScope] || "tickets.public_views";
    if (!(await userHasAnyPermission(req.user, [publicViewsKey]))) {
      return res.status(403).json({
        error: "You do not have permission to edit the public column view.",
        code: "PERMISSION_DENIED"
      });
    }
    const community = isCommunity();
    const defaults = [...DEFAULT_TICKET_TABLE_COLUMNS_BY_SCOPE[pageScope]];
    const columns = normalizeTicketTableColumns(req.body?.columns, {
      fallback: defaults,
      pageScope
    }) || [...defaults];
    const saved = await savePublicTicketTableColumns(columns, pageScope);
    const privateColumns = await loadPrivateTicketTableColumns(req.user?.id, pageScope);
    const resolved = resolveEffectiveTicketTableColumns({
      publicColumns: saved,
      privateColumns,
...
router.put("/table-columns/private", verifyJWT, async (req, res) => {
  try {
    ...
    await upsertUserSetting(userId, getPrivateTicketTableColumnsSettingKey(pageScope), columns);
    const publicColumns = await loadPublicTicketTableColumns(pageScope);
    const resolved = resolveEffectiveTicketTableColumns({
      publicColumns,
...
router.delete("/table-columns/private", verifyJWT, async (req, res) => {
  try {
    ...
    await pool.query(
      ...
      [userId, getPrivateTicketTableColumnsSettingKey(pageScope)]
    );
    const publicColumns = await loadPublicTicketTableColumns(pageScope);
    const resolved = resolveEffectiveTicketTableColumns({
      publicColumns,
...
router.get("/:id", verifyJWT, [param("id").isUUID().withMessage("Invalid ID ticket")], async (req, res) => {
  ...
  try {
    ...
    try {
      const eventResult = await pool.query(`SELECT id, title, type, start, "end", assigned_user_id, client_id, description
         FROM v_b_events
         WHERE ticket_id = $1
         ORDER BY start ASC NULLS LAST
         LIMIT 1`, [ticket.id]);
      planningEvent = eventResult.rows?.[0] || null;
    } catch {
      planningEvent = null;
    }
    const validationRequests = await listTicketValidationRequests(ticket.id).catch(() => []);
    res.json({
      ...ticket,
      planningEvent,
      validationRequests
...
router.post("/:id/validation-requests", verifyJWT, requireAnyPermission("tickets_detail.request_validation", "sales_detail.request_validation"), [param("id").isUUID(), body("validatorUserId").isUUID(), body("message").optional({
  nullable: true
  ...
  try {
    ...
    if (isTicketLockedForEdits(ticket)) {
      return res.status(409).json({
        error: "This ticket is closed and can no longer be modified. Reopen it to make changes."
      });
    }
    const created = await createTicketValidationRequest({
      ticketId: ticket.id,
      ticket,
      requestedByUserId: req.user?.id || null,
      validatorUserId: req.body.validatorUserId,
    ...
    console.error("POST /tickets/:id/validation-requests:", err);
    return res.status(500).json({
      error: "Unable to create validation request"
    });
  }
});
router.patch("/:id/validation-requests/:requestId", verifyJWT, requireAnyPermission("tickets_detail.request_validation", "sales_detail.request_validation"), [param("id").isUUID(), param("requestId").isUUID(), body("validatorUserId").isUUID(), body("message").optional({
  nullable: true
}).isString()], async (req, res) => {
  const validationResponse = validationErrorOrNull(req, res);
  if (validationResponse) return;
  try {
    ...
    const updated = await updateTicketValidationRequest({
      ticketId: ticket.id,
      validationRequestId: req.params.requestId,
      actorUserId: req.user?.id || null,
      validatorUserId: req.body.validatorUserId,
      message: req.body.message,
      ticket
    ...
    console.error("PATCH /tickets/:id/validation-requests/:requestId:", err);
    return res.status(500).json({
      error: "Unable to update validation request"
    });
  }
});
router.post("/:id/validation-requests/:requestId/respond", verifyJWT, requireAnyPermission("tickets.edit", "tickets.manage", "tickets.view"), [param("id").isUUID(), param("requestId").isUUID(), body("decision").isIn(["approved", "rejected"]), body("responseMessage").optional({
  nullable: true
}).isString()], async (req, res) => {
  const validationResponse = validationErrorOrNull(req, res);
  if (validationResponse) return;
  try {
    ...
    const updated = await respondTicketValidationRequest({
      ticketId: ticket.id,
      validationRequestId: req.params.requestId,
      actorUserId: req.user?.id || null,
    ...
    console.error("POST /tickets/:id/validation-requests/:requestId/respond:", err);
    return res.status(500).json({
      error: "Unable to respond to validation request"
    });
  }
});
...
router.patch("/:id/ai-runbook", verifyJWT, requirePermission("tickets_detail.ai_runbook"), [param("id").isUUID().withMessage("Invalid ID ticket")], async (req, res) => {
  const validationResponse = validationErrorOrNull(req, res);
  if (validationResponse) return;
  try {
...
router.put("/:id", verifyJWT, requirePermission("tickets.edit"), [param("id").isUUID(), body("title").optional().notEmpty(), body("description").optional().isString(), body("status").optional().isIn(STATUS_VALUES), body("priority").optional().isIn(PRIORITY_VALUES), body("type").optional().isString(), body("category").optional({
  ...
  nullable: true,
  checkFalsy: true
}).isUUID(), body("isMajorIncident").optional().isBoolean(), body("contactSlots").optional().isArray(), body("equipmentInfo").optional().isObject(), body("progressPercent").optional({
  nullable: true
}).isInt({
  min: 0,
  max: 100
  ...
  try {
    ...
    if (hasSalesFormData && Object.prototype.hasOwnProperty.call(req.body, "salesFormData")) {
      ...
      const incoming = req.body.salesFormData && typeof req.body.salesFormData === "object" && !Array.isArray(req.body.salesFormData) ? req.body.salesFormData : {};
      const next = {
        ...current
      };
      if (Object.prototype.hasOwnProperty.call(incoming, "pmTasks")) {
        const tasks = Array.isArray(incoming.pmTasks) ? incoming.pmTasks : [];
        next.pmTasks = tasks.map((task, index) => {
          const label = String(task?.label || "").trim();
          if (!label) return null;
          const startAt = task?.startAt || task?.start_at || null;
      ...
      if (hasProgressPercent && Object.prototype.hasOwnProperty.call(incoming, "pmTasks")) {
        const tasks = Array.isArray(next.pmTasks) ? next.pmTasks : [];
        const derivedProgress = tasks.length === 0 ? 0 : Math.max(0, Math.min(100, Math.round(tasks.filter(task => task.done).length / tasks.length * 100)));
        const progressIdx = updates.findIndex(u => u.startsWith("progress_percent ="));
        if (progressIdx >= 0) {
          values[progressIdx] = derivedProgress;
        } else {
          updates.push(`progress_percent = $${p++}`);
          values.push(derivedProgress);
        }
      }
    }
    const nextType = Object.prototype.hasOwnProperty.call(req.body, "type") ? req.body.type : oldTicket.type;
...
router.delete("/:id", verifyJWT, requireAnyPermission("tickets_detail.delete", "sales_detail.delete"), [param("id").isUUID().withMessage("Invalid ID ticket")], async (req, res) => {
  const validationResponse = validationErrorOrNull(req, res);
  if (validationResponse) return;
  try {
    const {
...
router.patch("/:id/status", verifyJWT, requireTicketStatusPermission, [param("id").isUUID(), body("status").isIn(STATUS_VALUES), body("note").optional().isString(), body("consumeSupportCredit").optional().isBoolean(), body("supportCreditDebits").optional().isArray(), body("supportCreditDebits.*.packId").optional().isUUID(), body("supportCreditDebits.*.amount").optional().isInt({
  min: 1
...
router.patch("/:id/comments/:commentId", verifyJWT, requireTicketCommentEditPermission, [param("id").isUUID(), param("commentId").isUUID(), body("content").isString(), body("isInternal").optional().isBoolean(), body("removeAttachmentIds").optional().isArray(), body("removeAttachmentIds.*").optional().isUUID(), body("removeAttachmentPaths").optional().isArray(), body("removeAttachmentPaths.*").optional().isString()], async (req, res) => {
  const validationResponse = validationErrorOrNull(req, res);
  if (validationResponse) return;
  try {
    const {
    ...
    const content = String(req.body?.content || "").trim();
    const hasIsInternal = Object.prototype.hasOwnProperty.call(req.body || {}, "isInternal");
    ...
    const nextIsInternal = hasIsInternal ? Boolean(req.body.isInternal) : Boolean(existing.is_internal);
    ...
    const updateResult = await pool.query(`UPDATE v_b_ticket_comments
         SET content = $1, is_internal = $2, updated_at = NOW()
         WHERE id = $3 AND ticket_id = $4
         RETURNING id, ticket_id, author_user_id, content, is_internal, created_at, updated_at`, [content, nextIsInternal, commentId, id]);
    await pool.query("UPDATE v_b_tickets SET updated_at = NOW() WHERE id = $1", [id]);
    if (!nextIsInternal && Boolean(existing.is_internal) && (await hasTicketColumn("sla_info"))) {
      await ensureTicketSlaInfoStored(id);
    }
...
router.delete("/:id/comments/:commentId", verifyJWT, requireAnyPermission("tickets_detail.edit_messages", "sales_detail.edit_messages"), [param("id").isUUID(), param("commentId").isUUID()], async (req, res) => {
  const validationResponse = validationErrorOrNull(req, res);
  if (validationResponse) return;
  try {
    if (!isAdminUser(req)) {
...
router.post("/:id/tags", verifyJWT, requireAnyPermission("tickets.view", "sales.view"), [param("id").isUUID(), body("label").notEmpty().isString(), body("color").optional().isString()], async (req, res) => {
  const validationResponse = validationErrorOrNull(req, res);
  if (validationResponse) return;
  try {
    const {
...
router.delete("/:id/tags/:tagId", verifyJWT, requireAnyPermission("tickets.view", "sales.view"), [param("id").isUUID(), param("tagId").isUUID()], async (req, res) => {
  const validationResponse = validationErrorOrNull(req, res);
  if (validationResponse) return;
  try {
    const {
...
router.post("/:id/watchers", verifyJWT, requireAnyPermission("tickets.view", "sales.view"), [param("id").isUUID(), body("userId").isUUID()], async (req, res) => {
  const validationResponse = validationErrorOrNull(req, res);
  if (validationResponse) return;
  try {
    const {
...
router.delete("/:id/watchers/:userId", verifyJWT, requireAnyPermission("tickets.view", "sales.view"), [param("id").isUUID(), param("userId").isUUID()], async (req, res) => {
  const validationResponse = validationErrorOrNull(req, res);
  if (validationResponse) return;
  try {
    const {
...
router.post("/:id/assignees", verifyJWT, requireAnyPermission("tickets.view", "sales.view"), [param("id").isUUID(), body("userId").isUUID()], async (req, res) => {
  const validationResponse = validationErrorOrNull(req, res);
  if (validationResponse) return;
  try {
    const hasAssignees = await hasTicketAssigneesTable();
...
router.delete("/:id/assignees/:userId", verifyJWT, requireAnyPermission("tickets.view", "sales.view"), [param("id").isUUID(), param("userId").isUUID()], async (req, res) => {
  const validationResponse = validationErrorOrNull(req, res);
  if (validationResponse) return;
  try {
    const hasAssignees = await hasTicketAssigneesTable();
