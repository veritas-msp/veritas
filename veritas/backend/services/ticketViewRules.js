const TICKET_VIEW_FIELDS = new Set(["title", "description", "type", "category", "status", "priority", "channel", "client_id", "assigned", "assigned_user_id", "pending_validator_user_id", "requester_contact_id", "requester_user_id", "ticket_number", "tags"]);
const TICKET_VIEW_OPERATORS = new Set(["contains", "not_contains", "equals", "not_equals", "starts_with", "ends_with", "in", "not_in", "is_empty", "is_not_empty"]);
import { normalizeFilterRoot, ticketMatchesFilterRoot, validateFilterTreeNodes, collectRuleNodes } from "./ticketViewFilterTree.js";
function normalizeRules(rules = {}) {
  const raw = rules && typeof rules === "object" ? rules : {};
  const filterRoot = normalizeFilterRoot(raw.filterRoot, raw);
  const legacyCriteria = collectRuleNodes(filterRoot).map(({
    field,
    operator,
    value
  }) => ({
  ...
  const criteria = hasFilterTreeRules ? legacyCriteria : criteriaFromRaw.length > 0 ? criteriaFromRaw : legacyCriteria;
  return {
...
function getAssignedSearchText(ticket, context = {}) {
  const parts = [context.assigneeLabel, ticket?.assigned_email, ticket?.assignedEmail];
  if (Array.isArray(ticket?.assignees)) {
    ticket.assignees.forEach(entry => {
...
function getTicketTagLabels(ticket) {
  ...
  return tags.map(tag => String(tag?.label ?? tag ?? "").trim().toLowerCase()).filter(Boolean);
}
...
function evaluateCriterion(criterion = {}, ticket = {}, context = {}) {
  ...
  if (field === "assigned_user_id") {
    ...
    if (expected === "__current_user__" || expected === "me" || expected === "current_user") {
      expected = String(context.currentUserId || "").trim().toLowerCase();
    }
    const expectedList = parseListValue(criterion?.value).map(value => {
      const normalized = String(value).trim().toLowerCase();
      if (normalized === "__current_user__" || normalized === "me" || normalized === "current_user") {
        return String(context.currentUserId || "").trim().toLowerCase();
      }
      return normalized;
    }).filter(Boolean);
    if (operator === "is_empty") return assignedIds.length === 0;
    if (operator === "is_not_empty") return assignedIds.length > 0;
    if (!expected && operator === "equals") return false;
  ...
  if (field === "pending_validator_user_id") {
    const pendingValidatorIds = (Array.isArray(ticket?.validationRequests) ? ticket.validationRequests : [])
      .filter(request => request?.isPending || String(request?.status || "").toLowerCase() === "pending")
      .map(request => String(request?.validatorUserId || request?.validator_user_id || "").trim().toLowerCase())
      .filter(Boolean);
    let expected = String(criterion?.value ?? "").trim().toLowerCase();
    if (expected === "__current_user__" || expected === "me" || expected === "current_user") {
      expected = String(context.currentUserId || "").trim().toLowerCase();
    ...
    if (operator === "is_empty") return pendingValidatorIds.length === 0;
    if (operator === "is_not_empty") return pendingValidatorIds.length > 0;
    if (!expected && operator === "equals") return false;
    if (operator === "equals") return pendingValidatorIds.includes(expected);
    if (operator === "not_equals") return !pendingValidatorIds.includes(expected);
    if (operator === "in") {
      if (expectedList.length === 0) return false;
      return pendingValidatorIds.some(id => expectedList.includes(id));
    }
    if (operator === "not_in") {
      if (expectedList.length === 0) return true;
      return !pendingValidatorIds.some(id => expectedList.includes(id));
...
export { TICKET_VIEW_FIELDS, TICKET_VIEW_OPERATORS, normalizeRules, getFieldValue, evaluateCriterion, ticketMatchesViewRules, validateViewRules, validateCriterion };
