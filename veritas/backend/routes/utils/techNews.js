...
import { ALLOWED_FEED_CATEGORIES, ALLOWED_FEED_LOCALES, createTechNewsFeed, deleteTechNewsFeed, getTechNewsFeedsMeta, listTechNewsFeeds, resetTechNewsFeeds, updateTechNewsFeed } from "../../utils/techNewsFeedsConfig.js";
...
router.get("/", verifyJWT, async (_req, res) => {
  try {
    const data = await getTechNewsFeed();
    res.json(data);
  } catch (err) {
